...
+
. `mvn wildfly:deploy`
. Access the application at http://localhost:8080/endpoint-security/, use `u1` username and `p1` password. All others are incorrect.
...
----
+
Configure SSL in WildFly per https://docs.jboss.org/author/pages/viewpage.action?pageId=66322705
+
[source, text]
----
...
<security-realm name="WebSocketRealm">
	<server-identities>
    	<ssl protocol="TLS">
        	<keystore path="websocket.keystore" relative-to="jboss.server.config.dir" keystore-password="websocket" alias="websocket"/>
        </ssl>
    </server-identities>
    <authentication>
        <truststore path="websocket.keystore" relative-to="jboss.server.config.dir" keystore-password="websocket"/>
    </authentication>
</security-realm>
----
+
...
. Access http://localhost:8080/endpoint-wss
.. See how the request is redirected to https
...
## Embedded WebSocket using Undertow/WildFly
